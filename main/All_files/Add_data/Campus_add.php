...
<body class="dark">
    ...
    <div class="sidebar">
        ...
        <ul class="side-menu">
            ...
            <li class="active"><a href="../dashboard/Campus.php"><i class='bx bx-store-alt'></i>Campus</a></li>
    ...
    <div class="content">
        ...
        <main>
            ...
            <form class="login-form" action="Campus_add.php" method="post">
                <h1><i class='bx bx-user'></i>  Add Campus</h1>
                ...
                <div class="flex-container">
                    <div class="form-input-material">
                        <input type="text" name="Campus_id" id="username1" placeholder=" " autocomplete="off"
                        class="form-control-material" required />
                        <label for="username1">Campus Id</label>
                    </div>

                    <div class="form-input-material">
                        <input type="text" name="campus_name" id="username2" placeholder=" " autocomplete="off"
                        class="form-control-material" required />
                        <label for="username2">Campus Name</label>
                    </div>
                </div>

                <div class="address">
                    <div class="form-input-material">
                        <input type="text" name="address" id="user2" placeholder=" " autocomplete="off"
                            class="form-control-material" required />
                        <label for="user2">Address</label>
                    </div>
                </div>

                <div class="flex-container">
                    <div class="form-input-material">
                        <input type="text" name="distance" id="username4" placeholder="" autocomplete="off"
                        class="form-control-material" required />
                        <label for="username4">Distance Form</label>
                    </div>

                    <div class="form-input-material">
                        <input type="text" name="Bus_route" id="username5" placeholder=" " autocomplete="off"
                        class="form-control-material" required />
                        <label for="username5">Bus Route</label>
                    </div>
                </div>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $id = filter_input(INPUT_POST, "Campus_id", FILTER_SANITIZE_STRING);
    $name = filter_input(INPUT_POST, "campus_name", FILTER_SANITIZE_STRING);
    $address = filter_input(INPUT_POST, "address", FILTER_SANITIZE_STRING);
    $route = filter_input(INPUT_POST, "Bus_route", FILTER_SANITIZE_STRING);
    $distance = filter_input(INPUT_POST, "distance", FILTER_VALIDATE_FLOAT);

    if ($id && $name && $address && $route !== null && $distance !== false) {
        // Use prepared statements to prevent SQL injection
        $sql = "INSERT INTO campus (Campus_id, campus_name, address, distance_to_city_center, bus_route) VALUES (?, ?, ?, ?, ?)";
        $stmt = mysqli_prepare($conn, $sql);
        mysqli_stmt_bind_param($stmt, "issss", $id, $name, $address, $distance, $route);
        
        if (mysqli_stmt_execute($stmt)) {
            $_SESSION["status"] = "Success";
            $_SESSION["text"] = "Data successfully recorded";
        } else {
            if (mysqli_errno($conn) == 1062) { // Check if error is due to duplicate key constraint
                $_SESSION["status"] = "error";
                $_SESSION["text"] = "Error: Duplicate entry. This ID already exists.";
            } else {
                $_SESSION["status"] = "error";
                $_SESSION["text"] = "An error occurred while processing your request. Please try again later.";
            }
        }
