...
<body class="dark">
    ...
    <div class="content">
        ...
        <main>
            ...
            <!-- Form start -->
            <form class="login-form" action="course_add.php" method="post">
                <h1><i class='bx bx-user'></i>  Add Course</h1>
                ...
                <div class="option-bars">
                    <div class="dropdown">
                        <div class="select">
                            <span class="selected">Programme Id</span>
                            <div class="caret"></div>
                        </div>
                        <ul class="menu">
                             <?php
                                $recent_club = "SELECT * FROM programme"; // Assuming 'students' is your table name
                                ...
                                if ($result) {
                                    if (mysqli_num_rows($result) > 0) {
                                        while ($row = mysqli_fetch_assoc($result)) {
                                            ...
                                            echo '<input type="radio" name="programme_id" value="' . $row['programme_id'] . '" id="' . $row['programme_id'] . '">';
                                            echo '<label for="' . $row['programme_id'] . '">' . $row['programme_id'] .'</label>';
                                            echo '</li>';
                                        }
                                    } else {
                    ...
                    <div class="dropdown">
                        <div class="select">
                            <span class="selected">Prerequisite Course Id (If Any)</span>
                            <div class="caret"></div>
                        </div>
                        <ul class="menu">
                            <?php
                                ...
                                if ($result) {
                                    if (mysqli_num_rows($result) > 0) {
                                        while ($row = mysqli_fetch_assoc($result)) {
                                            echo '<li class="option">';
                                            echo '<input type="radio" name="Cour_id" value="' . $row['course_id'] . '" id="' . $row['course_id'] . '">';
                                            echo '<label for="' . $row['course_id'] . '">' . $row['course_id'] .'</label>';
                                            echo '</li>';
                                        }
                                    } else {
                ...
                <div class="flex-container">
                    <div class="form-input-material">
                        <input type="text" name="course_id" id="username1" placeholder=" " autocomplete="off"
                        class="form-control-material" required />
                        <label for="username1">Course Id</label>
                    </div>

                    <div class="form-input-material">
                        <input type="text" name="course_code" id="username2" placeholder=" " autocomplete="off"
                        class="form-control-material" required />
                        <label for="username2">Course code</label>
                    </div>
                </div>
                ...
                <div class="address">
                    <div class="form-input-material">
                        <input type="text" name="course_title" id="user2" placeholder=" " autocomplete="off"
                            class="form-control-material" required />
                        <label for="user2">Course Title</label>
                    </div>
                </div>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($programme_id) && isset($id) && isset($course_name) && isset($course_code)){
            if(isset($_POST["Cour_id"])){
                $prerequisite_id = $_POST["Cour_id"];
                $sql = "INSERT INTO course (`course_id`, `course_code`, `course_title`, `programme_id`, `prerequisite_course_id`) VALUES ('$id', '$course_code', '$course_name', '$programme_id', '$prerequisite_id')";
        }else{
            $sql = "INSERT INTO course (`course_id`, `course_code`, `course_title`, `programme_id`) VALUES ('$id', '$course_code', '$course_name', '$programme_id')";
        }
