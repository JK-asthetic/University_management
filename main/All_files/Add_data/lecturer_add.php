...
<body class="dark">
    ...
    <div class="content">
        ...
        <main>
            ...
            <!-- Form start -->
            <form class="login-form" action="lecturer_add.php" method="post">
                <h1><i class='bx bx-user'></i>  Add Lecturer</h1>
                ...
                <div class="dropdown">
                        <div class="select">
                            <span class="selected">School ID</span>
                            <div class="caret"></div>
                        </div>
                        <ul class="menu">
                            <?php
                                $recent_club = "SELECT * FROM school"; // Assuming 'students' is your table name
                                ...
                                if ($result) {
                                    if (mysqli_num_rows($result) > 0) {
                                        while ($row = mysqli_fetch_assoc($result)) {
                                            ...
                                            echo '<input type="radio" name="school_id" value="' . $row['school_id'] . '" id="' . $row['school_id'] . '">';
                                            echo '<label for="' . $row['school_id'] . '">' . $row['school_id'] .'</label>';
                                            echo '</li>';
                                        }
                                    } else {
                                        echo '<li>No Campus found.</li>';
                                    }
                    ...
                    <div class="dropdown">
                        <div class="select">
                            <span class="selected">Supervisor ID</span>
                            <div class="caret"></div>
                        </div>
                        <ul class="menu">
                            <?php
                                $recent_club = "SELECT * FROM lecturer"; // Assuming 'students' is your table name
                                $result = mysqli_query($conn, $recent_club);
                                ...
                                if ($result) {
                                    if (mysqli_num_rows($result) > 0) {
                                        while ($row = mysqli_fetch_assoc($result)) {
                                            echo '<li class="option">';
                                            echo '<input type="radio" name="supervisor_id" value="' . $row['lecturer_id'] . '" id="' . $row['lecturer_id'] . '">';
                                            echo '<label for="' . $row['lecturer_id'] . '">' . $row['lecturer_id'] .'</label>';
                                            echo '</li>';
                                        }
                                    } else {
                                        echo '<li>No Lecturer found.</li>';
                                    }
                                    mysqli_free_result($result);
                                } else {
                                    echo 'Error: ' . mysqli_error($conn);
                ...
                <div class="flex-container">
                    <div class="form-input-material">
                        <input type="text" name="lecturer_name" id="username5" placeholder=" " autocomplete="off"
                        class="form-control-material" required />
                        <label for="username5">Lecturer Name</label>
                    </div>
                                
                    <div class="form-input-material">
                        <input type="text" name="title" id="user2" placeholder=" " autocomplete="off"
                            class="form-control-material" required />
                ...
                <div class="flex-container">
                    ...
                    <div class="form-input-material" style = "width:460px;">
                        <input type="text" name="Room_no" id="username2" placeholder=" " autocomplete="off"
                        class="form-control-material" required />
                        <label for="username2">Room no</label>
                    </div>
                </div>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql_check_empty = "SELECT COUNT(*) as count FROM lecturer";
    $result_check_empty = $conn->query($sql_check_empty);

    if ($result_check_empty) {
        $row = $result_check_empty->fetch_assoc();
        echo "Count of lecturer rows: " . $row['count'] . "<br>";
        if ($row['count'] == 0) {
            $lecturer_id = $lecturer_prefix . str_pad($starting_number, 3, '0', STR_PAD_LEFT);
        } else {
            $sql_get_latest_id = "SELECT MAX(lecturer_id) as max_id FROM lecturer";
            $result_get_latest_id = $conn->query($sql_get_latest_id);
            if ($result_get_latest_id) {
                $row_latest_id = $result_get_latest_id->fetch_assoc();
                $latest_id = $row_latest_id['max_id'];
                echo "Latest lecturer ID: " . $latest_id . "<br>";
                $latest_number = intval(substr($latest_id, strlen($lecturer_prefix)));
                $next_number = $latest_number + 1;
                $lecturer_id = $lecturer_prefix . str_pad($next_number, 3, '0', STR_PAD_LEFT);
            } else {
                echo "Error fetching latest lecturer ID: " . $conn->error . "<br>";
            }
        }}

    echo $lecturer_id;
    $school_id = $_POST["school_id"];
    $location = $_POST["Room_no"];
    $name = $_POST["lecturer_name"];
    $title = $_POST["title"];

    if (isset($lecturer_id) && isset($location) && isset($name) && isset($school_id) && isset($title)){
        if(isset($_POST['supervisor_id'])){
            $sup_id = $_POST["supervisor_id"];
            $sql = "INSERT INTO lecturer (`lecturer_id`, `lecturer_name`, `title`, `office_room`, `school_id`, `supervisor_id`)  VALUES ('$lecturer_id', '$name', '$title', '$location', '$school_id', '$sup_id')";
        }else{
            $sql = "INSERT INTO lecturer (`lecturer_id`, `lecturer_name`, `title`, `office_room`, `school_id`)  VALUES ('$lecturer_id', '$name', '$title', '$location', '$school_id')";
        }
