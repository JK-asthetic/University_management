...
$query = "SELECT `campus_name`, `address`, `distance_to_city_center`, `bus_route` FROM `campus` WHERE campus_id = ?";
$stmt = $conn->prepare($query);
$stmt->bind_param("i", $campus_id);
$stmt->execute();
$stmt->store_result(); // Store the result for later use
if ($stmt->num_rows > 0) { // Check if rows are found
    $stmt->bind_result($campus_name, $address, $distance_to_city_center, $bus_route);
    $stmt->fetch();
} else {
    $_SESSION["status"] = "error";
    $_SESSION["text"] = "campus not found";
...
<body class="dark">
    ...
    <div class="content">
        ...
        <main>
            ...
            <form class="login-form" action="Campus_update_3.php" method="post">
                <h1><i class='bx bx-user'></i>  Add Campus</h1>


                <div class="flex-container">
                    <div class="form-input-material">
                        <input type="text" name="Campus_name" id="username1" placeholder=" " autocomplete="off"
                            class="form-control-material" required value="<?php echo $campus_name; ?>">
                        <label for="username1">Campus Name</label>
                    </div>

                    <div class="form-input-material">
                        <input type="text" name="address" id="username2" placeholder=" " value = "<?php echo $address; ?>" autocomplete="off"
                            class="form-control-material" required />
                        <label for="username2">Address</label>
                    </div>
                </div>

                <div class="flex-container">
                    <div class="form-input-material">
                        <input type="text" name="distance" id="username4" value = "<?php echo $distance_to_city_center; ?>" placeholder="" autocomplete="off"
                        class="form-control-material" required />
                        <label for="username4">Distance Form</label>
                    </div>

                    <div class="form-input-material">
                        <input type="text" name="Bus_route" id="username5" value = "<?php echo $bus_route; ?>" placeholder="" autocomplete="off"
                        class="form-control-material" required />
