...
$query = "SELECT  `committee_title`, `faculty_id`, `meeting_frequency` FROM `committee` WHERE committee_id = ?";
$stmt = $conn->prepare($query);
$stmt->bind_param("i", $committee_id);
$stmt->execute();
$stmt->store_result(); // Store the result for later use
if ($stmt->num_rows > 0) { 
    $stmt->bind_result($committee_title, $faculty_id, $meeting_time);
    $stmt->fetch();
} else {
    $_SESSION["status"] = "error";
    $_SESSION["text"] = "committee not found";
...
<body class="dark">
    ...
    <div class="content">
        ...
        <main>
            ...
            <form class="login-form" action="committee_update_3.php" method="post">
                <h1><i class='bx bx-user'></i>Update committee</h1>

                <div class="flex-container">
                    <div class="form-input-material">
                        <input type="text" id="username1" value =  "<?php echo $committee_id; ?>" 
                        class="form-control-material"/>
                        <label for="username1">Committee ID</label>
                    </div>

                    <div class="form-input-material">
                        <input type="text" id="username6" value = "<?php echo $faculty_id; ?>"
                        class="form-control-material" required />
                        <label for="username6">Faculty ID</label>
                    </div>
                ...
                <div class="flex-container">
                    <div class="form-input-material">
                        <input type="text" name="title" id="username2" value =  "<?php echo $committee_title; ?>"
                        class="form-control-material"/>
                        <label for="username2">Club Name</label>
                    </div>

                    <div class="form-input-material">
                        <input type="text" name="frequency" id="username7" value = "<?php echo $meeting_time; ?>" placeholder=" " autocomplete="off"
                        class="form-control-material" required/>
                        <label for="username7">Meeting Frequency</label>
                    </div>
