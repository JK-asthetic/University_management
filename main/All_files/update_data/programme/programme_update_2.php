...
$_SESSION['programme_id'] = $_POST['programme_id'];
include("../../php_files/Database.php");


$programme_id = $_POST['programme_id'];

$query = "SELECT `programme_code`, `programme_title`, `level`, `duration`, `school_id` FROM `programme` WHERE programme_id = ?";
$stmt = $conn->prepare($query);
$stmt->bind_param("i", $programme_id);
$stmt->execute();
$stmt->store_result(); // Store the result for later use
if ($stmt->num_rows > 0) { 
    $stmt->bind_result($code, $programme_title, $level, $duaration, $school_id);
    $stmt->fetch();
} else {
    $_SESSION["status"] = "error";
    $_SESSION["text"] = "programme not found";
...
<body class="dark">
    ...
    <div class="content">
        ...
        <main>
            ...
            <form class="login-form" action="programme_update_3.php" method="post">
                <h1><i class='bx bx-user'></i>Update Lecturer</h1>

                <div class="flex-container">
                    <div class="form-input-material">
                        <input type="text" id="username1" value =  "<?php echo $programme_id; ?>" 
                        class="form-control-material"/>
                        <label for="username1">Programme ID</label>
                    </div>

                    <div class="form-input-material">
                        <input type="text" id="username5" value =  "<?php echo $school_id; ?>"
                        class="form-control-material"/>
                        <label for="username5">School ID</label>
                    </div>
                ...
                <div class="flex-container">
                    ...
                    <div class="form-input-material">
                        <input type="text" id="username2" name="code" value =  "<?php echo $code; ?>"
                        class="form-control-material" required />
                        <label for="username2">Code</label>
                    </div>

                    <div class="form-input-material">
                        <input type="text" name="title" id="user2" value = "<?php echo $programme_title; ?>" placeholder=" " autocomplete="off"
                            class="form-control-material" required />
                ...
                <div class="flex-container">
                    ...
                    <div class="form-input-material">
                        <input type="text" id="username3" name = "level" value =  "<?php echo $level; ?>"
                        class="form-control-material"/>
                        <label for="username3">Code</label>
                    </div>

                    <div class="form-input-material">
                        <input type="text" name="duration" id="user3" value = "<?php echo $duaration; ?>" placeholder=" " autocomplete="off"
                            class="form-control-material" required />
                        <label for="user3">Duration</label>
                    </div>
